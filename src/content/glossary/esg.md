---
term: "ESG"
title: "ESG: Definition, Solar Role & Indian Corporate Applications"
description: "ESG covers Environmental, Social and Governance reporting. Full guide to solar's role in corporate ESG."
category: "ESG"
categorySlug: "esg"
priority: "P1"
updatedDate: 2026-07-08
keywords:
  - what is esg
  - esg solar
  - corporate sustainability
  - net zero india
  - esg reporting
shortDefinition: "ESG (Environmental, Social, Governance) is the framework for corporate sustainability reporting. Solar adoption is the most visible E lever for Indian companies, contributing to Scope 2 emissions reduction and net-zero targets. BRSR is the Indian mandatory ESG reporting standard for listed companies."
quickFacts:
  industry: "Corporate Sustainability"
  primaryUse: "Sustainability reporting and investment screening"
  commonUsers: "Listed corporates, investors, regulators"
  relevantStandards: "BRSR, GRI, SASB, TCFD, ISSB IFRS S1/S2"
  relatedTechnologies: "Solar PV, renewable energy certificates, carbon accounting"
relatedTerms:
  - { slug: "rec", term: "Renewable Energy Certificate" }
  - { slug: "rpo", term: "RPO" }
  - { slug: "green-hydrogen", term: "Green Hydrogen" }
  - { slug: "ev-charger", term: "EV Charger" }
  - { slug: "ppa", term: "Power Purchase Agreement" }
  - { slug: "resco", term: "RESCO" }
  - { slug: "open-access", term: "Open Access" }
  - { slug: "captive-plant", term: "Captive Plant" }
  - { slug: "lcoe", term: "LCOE" }
  - { slug: "rooftop-solar", term: "Rooftop Solar" }
  - { slug: "carbon-credit", term: "Carbon Credit" }
  - { slug: "net-zero", term: "Net Zero" }
  - { slug: "scope-2-emissions", term: "Scope 2 Emissions" }
faqs:
  - q: "What is ESG in simple words?"
    a: "A way to measure how a company impacts the environment, treats people and is governed. Solar is the most direct way to improve the E (environmental) score."
  - q: "What does ESG stand for?"
    a: "Environmental, Social, Governance."
  - q: "Is ESG mandatory in India?"
    a: "BRSR (Business Responsibility and Sustainability Report) is mandatory for top 1,000 listed companies since FY 2022-23."
  - q: "How does solar improve ESG?"
    a: "Direct reduction in Scope 2 emissions (purchased electricity). Visible sustainability marker. Contributes to renewable energy share."
  - q: "What are Scope 1, 2, 3?"
    a: "Scope 1: direct emissions (e.g., gas boiler). Scope 2: purchased electricity. Scope 3: value chain. Solar reduces Scope 2 directly."
  - q: "What is net zero?"
    a: "Balancing emissions with removals to reach zero net. India committed to net zero by 2070 at COP26."
  - q: "What is the ISSB standard?"
    a: "International Sustainability Standards Board's IFRS S1 (general) and S2 (climate). Becoming global standard for sustainability reporting."
  - q: "What is BRSR Core?"
    a: "Subset of BRSR covering key disclosures including renewable energy share. Assured by independent assessors for top 150 listed companies."
  - q: "What is GRI?"
    a: "Global Reporting Initiative, voluntary global standard for sustainability reporting."
  - q: "What is TCFD?"
    a: "Task Force on Climate-related Financial Disclosures, framework for climate-related risk reporting. Now absorbed into ISSB."
  - q: "How is renewable energy share calculated?"
    a: "Renewable kWh / Total energy consumed × 100. Includes solar, wind, hydro and biomass."
  - q: "Do companies need to verify ESG data?"
    a: "BRSR Core requires assurance. Voluntary ESG reports often verified by third parties."
  - q: "What is SBTi?"
    a: "Science Based Targets initiative, sets emission reduction targets aligned with 1.5°C climate goal."
  - q: "Can solar replace coal in ESG?"
    a: "For purchased electricity, yes. Captive coal power replaced by solar reduces Scope 1 emissions."
  - q: "What is greenwashing?"
    a: "Misleading claims about sustainability without substantive action. Real solar adoption avoids this."
author: "Nirav Dhanani"
---

## What is ESG

ESG, Environmental, Social and Governance, is the framework for assessing and reporting corporate sustainability performance. Environmental covers climate impact, resource use, waste and biodiversity. Social covers labour practices, community relations, human rights and product responsibility. Governance covers board structure, executive compensation, risk management and business ethics.

For corporates, ESG reporting has shifted from voluntary to increasingly mandatory. India's BRSR (Business Responsibility and Sustainability Report) is mandatory for the top 1,000 listed companies since FY 2022-23. BRSR Core, a subset requiring third-party assurance, applies to the top 150 listed companies.

Solar adoption is the most visible and quantifiable lever for the E (environmental) component. Solar directly reduces Scope 2 emissions (from purchased electricity) and supports corporate net-zero commitments. Indian listed companies report renewable energy share in their BRSR submissions, making solar adoption a tracked metric.

> **TL;DR**
> - ESG (Environmental, Social, Governance) is the framework Indian corporates use to report sustainability performance, with BRSR mandatory for the top 1,000 listed companies since FY 2022-23.
> - Solar is the most direct, auditable lever for the E score, cutting Scope 2 emissions from purchased electricity.
> - A 5 kW rooftop system avoids about 5.3 tonnes CO2 a year; a 1 MW C&I plant avoids roughly 1,200 tonnes annually, using India's grid emission factor of 0.71 kgCO2/kWh.
> - BRSR Core, applicable to the top 150 listed companies, requires third-party assurance of ESG data.
> - Common Indian C&I targets sit at 30 to 50 percent renewable energy share by 2030, tracked through metered solar generation.
> - Combining on-site solar with PPAs, RECs and RESCO models is the standard way corporates build a credible, greenwashing-resistant ESG strategy.

## Why ESG matters

Regulatory compliance. BRSR is mandatory for Indian listed companies.

Investor screening. ESG funds screen companies on E, S and G metrics.

Cost of capital. Strong ESG performers access cheaper finance, and lenders increasingly weigh a company's [cost of capital](/glossary/wacc/) against its renewable energy share.

Talent attraction. Younger workforce values ESG-aligned employers.

Customer demand. Sustainable supply chain requirements from B2B customers.

Risk management. Climate risks materially affect business.

## ESG and solar

| ESG dimension | Solar contribution |
|---|---|
| Scope 1 reduction | Replacing diesel gensets |
| Scope 2 reduction | Replacing grid electricity |
| Scope 3 reduction | Through supply chain pressure |
| RE share metric | Direct contribution |
| Carbon abatement | Quantified per kWh generated |
| Net-zero alignment | Visible step toward commitment |
| Reporting | Direct, auditable kWh contribution |

Corporates typically start with a turnkey commercial solar installation sized against annual grid consumption, then layer in a [power purchase agreement](/glossary/ppa/) for loads beyond rooftop capacity or a [RESCO](/glossary/resco/) partner that owns and maintains the plant on a zero-capex basis. For sizing and return calculations at C&I scale, see [Solar IRR and Payback for C&I in India](/blog/solar-irr-payback-ci-india/).

## Technical deep dive

### Indian BRSR structure

| Section | Content |
|---|---|
| Section A | Company profile |
| Section B | Material ESG risks |
| Section C | Principle-wise performance disclosure (NGRBC principles) |

NGRBC Principles include environment, ethics, employee welfare and stakeholder responsiveness.

### Scope 2 emission factor

Indian grid emission factor (2024 approximate): 0.71 kgCO2/kWh.

Each MWh of grid electricity replaced by solar avoids 0.71 tonnes CO2.

A [5 kW residential rooftop system](/blog/5kw-solar-system-price-india/) generating 7,500 kWh/year avoids 5.3 tonnes CO2.

A 1 MW [C&I solar plant](/c-i-solution/) avoids 1,200 tonnes CO2 annually.

### Indian ESG reporting standards

| Standard | Application |
|---|---|
| BRSR | Mandatory for top 1,000 listed |
| BRSR Core | Mandatory with assurance for top 150 |
| GRI | Voluntary global standard |
| TCFD / ISSB IFRS S1/S2 | Climate-related disclosures |
| CDP | Climate Disclosure Project, voluntary |
| SBTi | Science-based targets |

### Solar contribution to RE share

For a company:
RE share (%) = (Solar kWh + Other RE kWh) / Total electricity kWh × 100

Common Indian C&I corporate targets: 30 to 50 percent RE share by 2030. Corporates modelling the glide path to that target often use [solar financial modeling tools](https://surgepv.com/generation-financial-tool/) to project savings against BRSR-linked commitments.

## Real-world applications

BRSR reporting by listed companies.

Sustainability reports for unlisted companies seeking investment.

Carbon footprint quantification.

Investor presentations to ESG funds.

Supply chain sustainability disclosures.

[Tender bid documents](/blog/how-to-evaluate-solar-epc-bids/) with ESG scoring.

## Advantages of solar for ESG

Direct, auditable, quantifiable contribution.

Visible to all stakeholders.

Aligned with global frameworks.

Cost reduction alongside ESG benefit, aligned with a favourable [LCOE](/glossary/lcoe/) trend.

Long-term commitment (25-year asset).

## Limitations

ESG reporting is one tool, not the whole strategy.

Variability across reporting frameworks.

Risk of greenwashing if claims are not substantive.

Data quality challenges in supply chain Scope 3.

## Common ESG and solar problems

Overstating renewable energy share through accounting tricks.

Buying RECs without on-site solar adoption.

Insufficient measurement of solar generation.

Greenwashing claims unsupported by data.

Failure to align ESG strategy with operational practice.

## Best practices

Combine on-site solar with off-site PPAs and [RECs](/glossary/rec/) as needed.

Use third-party verified emission data.

Report consistently across multiple frameworks.

Align ESG targets with measurable solar adoption.

Document Scope 2 reduction with metered solar data.

Integrate ESG into business strategy, not just reporting.

## Indian market context

BRSR became mandatory in FY 2022-23 for top 1,000 listed companies.

BRSR Core mandatory with assurance for top 150 from FY 2023-24.

SEBI also mandates sustainability disclosures for IPOs and rights issues.

Major Indian corporates with significant solar adoption: Tata Group, Reliance Industries, Mahindra Group, Infosys, Wipro, TCS.

RE100 (renewable electricity 100 percent target) membership growing among Indian corporates, reflected in broader [India solar statistics](/blog/india-solar-statistics-2026-data/) for 2026.

Multi-site corporate rollouts often require MW-scale EPC project management to keep BRSR-linked installation timelines on track. Some large industrial ESG programmes now extend decarbonisation beyond electricity to [green hydrogen](/glossary/green-hydrogen/) for hard-to-abate processes and to [EV charger](/glossary/ev-charger/) rollouts for fleet electrification.

## Standards and certifications

| Standard | Scope |
|---|---|
| BRSR / BRSR Core | Indian mandatory |
| ISSB IFRS S1, S2 | Global standard |
| GRI Standards | Voluntary global |
| CDP | Climate disclosure |
| SBTi | Science-based targets |
| RE100 | Renewable electricity commitment |

## Common mistakes

Reporting renewable share without verification.

Buying low-quality offsets.

Greenwashing claims.

Inconsistent reporting across years.

Failure to align ESG with operational practice.

## Key takeaways

ESG is the framework for corporate sustainability reporting. Indian BRSR is mandatory for top 1,000 listed companies. Solar is the most direct lever for Scope 2 emission reduction and renewable energy share. Quantifiable, auditable contribution to environmental targets. Combined with strong S and G performance, ESG-aligned companies access lower cost of capital, better investor relations and stronger talent attraction.

## Need solar-backed ESG strategy?

QBits Energy structures solar adoption pathways for Indian corporates pursuing BRSR, net-zero and RE100 targets through rooftop solar, open access PPAs and RESCO models.

## Further reading

For how ESG plays out in real projects, these guides go deeper:

- [Case Study, 200 kW Solar at a Chennai IT Park](/blog/case-study-chennai-it-park/)
- [Net Metering in India, Complete 2026 Guide](/blog/net-metering-india-complete-guide/)
- [Best Solar Design Software Philippines: 2026 Guide](/blog/solar-design-software-philippines/)
- [Best Solar Design Software Saudi Arabia: 2026 Guide](/blog/solar-design-software-saudi-arabia/)

## Sources

- SEBI BRSR Framework, Securities and Exchange Board of India.
- BRSR Core Reporting Format.
- GRI Standards, Global Reporting Initiative.
- ISSB IFRS S1 and S2.
- TCFD Recommendations.
- CDP Climate Disclosure.
- Science Based Targets initiative (SBTi).
- RE100 Annual Disclosure.
- IEA Renewables 2024.
