...
keywords:
  ...
  - KUSUM solar pump
  - solar pump controller India
  - 1 HP solar pump inverter
  - agricultural solar pump sizing
faqs:
  - q: "Can a standard on-grid solar inverter run a water pump directly?"
    a: "A standard on-grid inverter will supply power to any AC load connected to its output, including a water pump motor. However, pumps draw 5 to 7 times their rated current at start-up (inrush current). If the inverter does not have sufficient overload capacity, it will trip or shut down on the surge. Most on-grid inverters allow 1.1 to 1.5× continuous overload. For reliable pump operation, the inverter should be rated at least 1.5 to 2× the pump motor's kW rating, or a dedicated pump controller with a Variable Frequency Drive (VFD) should be used to soft-start the motor."
  - q: "What is KUSUM Component-B and how does it relate to solar pumps?"
    a: "KUSUM Component-B is the sub-scheme under PM-KUSUM that funds standalone solar pumps for agricultural irrigation. Under Component-B, the central government pays 30% of the pump system cost, the state government pays 30%, and the farmer pays only 10% as upfront contribution while the remaining 30% is financed through a concessional bank loan from NABARD or designated rural banks. Coverage is for solar pumps from 0.5 HP to 7.5 HP. The system operates entirely off the grid, so no DISCOM approval is required for installation."
  - q: "How do I convert HP to kW for pump sizing?"
    a: "One horsepower equals 0.746 kilowatts. A 1 HP motor draws 0.746 kW at full load; a 2 HP motor draws 1.49 kW; a 5 HP motor draws 3.73 kW. However, motor efficiency and power factor mean the actual input power drawn from the inverter is higher. A 1 HP motor with 80% efficiency draws roughly 0.93 kW from the supply. For solar sizing, always use the input power (not shaft power) and add at least 30% for inrush current headroom when selecting the inverter or pump controller capacity."
  - q: "What is inrush current and why does it matter for solar pump systems?"
    a: "Inrush current is the spike of current a motor draws in the first 100–300 milliseconds of starting. For an AC induction motor - which is the most common type used in agricultural submersible and surface pumps - inrush current is 5 to 7 times the full-load running current. A 1 HP motor running at 4 A full-load will surge to 20–28 A at start. A solar pump controller with Variable Frequency Drive (VFD) eliminates inrush by ramping up frequency slowly from near zero - this is why pump controllers are preferred over plain inverters for direct pump operation."
  - q: "Is a separate solar pump controller needed, or can I use a solar inverter?"
    a: "The answer depends on your setup. If you are running a grid-connected pump (where mains grid is the primary power and solar reduces the electricity bill), an on-grid solar inverter is correct - it feeds power to the grid or to your premises load, and the pump draws from the AC supply as usual. If you want a standalone off-grid pump that runs only on solar with no grid connection, a dedicated solar pump controller with VFD is the right product - it drives the motor directly from DC panels. Mixing these up is the most common mistake in agricultural solar procurement."
  - q: "Which KUSUM component applies to a farmer who already has a grid-connected pump?"
    a: "KUSUM Component-C applies to farmers with existing grid-connected agricultural pump connections. Under Component-C, solar panels are installed on the farm and connected to the existing pump feeder or a dedicated agricultural feeder. The solar generation powers the pump during daylight and surplus units are exported to the DISCOM at a state feed-in tariff. This differs from Component-B, which replaces diesel or manual pumps entirely with standalone solar systems. A grid-connected inverter compliant with DISCOM net-metering or gross-metering rules is required for Component-C setups."
  - q: "How many solar panels are needed for a 2 HP submersible pump?"
    a: "A 2 HP submersible pump motor draws approximately 1.49 kW shaft power, or about 1.86 kW input power at 80% motor efficiency. To run it for 6–8 peak sun hours per day, you need a solar array of at least 2.2 to 2.5 kWp, accounting for cable losses and panel temperature derating in Indian summer conditions (typically 15–20% loss at 45 °C ambient). In practice, MNRE-approved solar pump packages for 2 HP standalone systems use 2 to 3 kWp panel arrays paired with a pump controller rated at 2.2 kW output."
...
KUSUM Component-B systems use this topology. The system is self-contained: solar panels → pump controller → AC motor pump. No DISCOM approval is required.
...
| --- | --- | --- |
| Grid connection required? | No (standalone) | Yes |
| Drives the motor directly? | Yes (DC panels → VFD → motor) | No (feeds grid or premises bus) |
...
| Pump Rating | Shaft Power (kW) | Input Power at 80% eff. | Min VFD Controller Rating | Min On-Grid Inverter (if grid-tied) |
| --- | --- | --- | --- | --- |
| 0.5 HP | 0.37 kW | 0.47 kW | 0.75 kW | 1.0 kW |
| 1 HP | 0.75 kW | 0.93 kW | 1.1 kW | 1.5 kW |
| 2 HP | 1.49 kW | 1.86 kW | 2.2 kW | 3.0 kW |
| 3 HP | 2.24 kW | 2.80 kW | 3.7 kW | 4.0 kW |
| 5 HP | 3.73 kW | 4.66 kW | 5.5 kW | 6.0–7.5 kW |
| 7.5 HP | 5.59 kW | 6.99 kW | 7.5 kW | 10 kW |
...
Component-B is the most relevant for most small and medium farmers. It subsidises the complete replacement of diesel pumps (or the installation of new solar pumps where no pump exists) with standalone solar pump systems rated 0.5 HP to 7.5 HP.
...
| Scenario | Component | Product Needed | Grid Required? |
| --- | --- | --- | --- |
| Replace diesel pump with solar | Component-B | Solar pump controller + panel array | No |
| New borewell on solar for irrigation | Component-B | Solar pump controller + panel array | No |
| Reduce electricity bill on existing electric pump | Component-C | On-grid solar inverter + panel array | Yes |
| Earn income from excess solar on farm feeder | Component-C | On-grid solar inverter + panel array | Yes |
| Lease farm land to solar developer | Component-A | Not applicable (developer's equipment) | Yes |

## Sizing the Solar Array for Agricultural Pumps

Sizing the solar array correctly determines whether the pump runs through peak irrigation hours or falls short on cloudy days. The array must cover the pump's daily energy demand within the available peak sun hours at the farm location.

### Calculating Daily Energy Demand

Start with how many hours per day the pump runs and the electrical input power (from the Pump Sizing Triangle, Step 2).

- A 2 HP pump (1.86 kW input) running 8 hours per day consumes 14.9 kWh.
- A 5 HP pump (4.66 kW input) running 8 hours per day consumes 37.3 kWh.

### Peak Sun Hours by Region

India's peak sun hours range from 4.5 hours per day in the Gangetic plain and North-East to 5.5–6.0 hours in Rajasthan, Gujarat, and Telangana. Use location-specific data from the [Solar Energy Centre / MNRE irradiation maps](https://mnre.gov.in/) or NREL's Global Solar Atlas.

For a conservative sizing exercise, use 4.5 peak sun hours and include a 20% system loss factor (cable losses, temperature derating at 45 °C, controller inefficiency). India's irradiation data by district is publicly available from the [NREL Global Solar Atlas](https://globalsolaratlas.info/) and from MNRE's Solar Radiation Resource Assessment stations.

### Array Sizing Formula

Array size (kWp) = Daily energy demand (kWh) ÷ (Peak sun hours × 0.80 system efficiency)

Applied to a 2 HP pump running 8 hours in Maharashtra (5.0 PSH):
...
| --- | --- | --- | --- | --- |
| Submersible (borewell) | 0.5–7.5 HP | 1-phase (≤2 HP), 3-phase (>2 HP) | 6–7× | Excellent with 3-phase VFD controller |
| Surface centrifugal (mono-block) | 0.5–5 HP | 1-phase (≤2 HP), 3-phase (>2 HP) | 5–6× | Excellent with matching VFD controller |
...
For most smallholder farmers without reliable grid supply (common in UP, Bihar, MP, Odisha, Jharkhand), Component-B standalone solar pumps are the most practical choice. For farms in Punjab, Haryana, Gujarat, and Maharashtra with stable agricultural feeder supply, Component-C grid-tied systems often deliver better ROI because they earn feed-in income and allow pump operation at any time.

Read the full [on-grid vs hybrid decision guide](/blog/on-grid-vs-hybrid-vs-off-grid-decision-guide/) for a detailed ROI comparison across all three topologies.

## Common Mistakes When Buying a Solar Pump System
...
- **At purchase**: Verify that the pump controller is MNRE-empanelled for KUSUM if you are claiming Component-B subsidy. Non-empanelled equipment, however technically sound, will not qualify for subsidy reimbursement.
