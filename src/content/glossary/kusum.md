---
term: "PM-KUSUM"
title: "PM-KUSUM: Definition, Components & Indian Solar Pump Scheme"
description: "PM-KUSUM is India's agricultural solar scheme. Full guide to Components A, B, C and benefits."
category: "Policies"
categorySlug: "policies"
priority: "P1"
updatedDate: 2026-07-08
keywords:
  - what is pm kusum
  - kusum scheme
  - solar pump india
  - kusum component a b c
  - agriculture solar
shortDefinition: "PM-KUSUM (Pradhan Mantri Kisan Urja Suraksha evam Utthaan Mahabhiyan) is India's flagship scheme for agricultural solar, launched in 2019. It targets 35 GW of solar capacity in agriculture by 2026 across three components: A (decentralised solar plants), B (standalone solar pumps), C (grid-connected solar pumps)."
quickFacts:
  industry: "Agricultural Solar Policy"
  primaryUse: "Solarising agriculture"
  commonUsers: "Farmers, state agencies, MNRE"
  relevantStandards: "MNRE PM-KUSUM scheme guidelines"
  relatedTechnologies: "Solar pumps, solar parks, grid feeders"
relatedTerms:
  - { slug: "mnre", term: "MNRE" }
  - { slug: "pm-surya-ghar", term: "PM Surya Ghar" }
  - { slug: "discom", term: "DISCOM" }
  - { slug: "off-grid-inverter", term: "Off Grid Inverter" }
  - { slug: "on-grid-inverter", term: "On Grid Inverter" }
  - { slug: "feed-in-tariff", term: "Feed In Tariff" }
  - { slug: "almm-list", term: "ALMM List" }
  - { slug: "monocrystalline", term: "Monocrystalline" }
  - { slug: "mppt", term: "MPPT" }
  - { slug: "net-metering", term: "Net Metering" }
  - { slug: "capex", term: "CAPEX" }
  - { slug: "irr", term: "IRR" }
  - { slug: "payback-period", term: "Payback Period" }
faqs:
  - q: "What is PM-KUSUM in simple words?"
    a: "A central scheme that subsidises solar for farmers: standalone solar pumps, solarisation of grid-connected pumps and decentralised solar plants on farm land."
  - q: "What does PM-KUSUM stand for?"
    a: "Pradhan Mantri Kisan Urja Suraksha evam Utthaan Mahabhiyan."
  - q: "What are the components?"
    a: "Component A: 10 GW of decentralised ground-mounted solar plants on barren farm land. Component B: 14 lakh standalone solar pumps. Component C: 35 lakh grid-connected pump solarisation."
  - q: "When was PM-KUSUM launched?"
    a: "March 2019."
  - q: "What is the target capacity?"
    a: "35 GW of solar capacity by 2026 across all three components."
  - q: "What is the subsidy structure?"
    a: "Up to 60 percent of capex split between central (30%) and state (30%). Farmer pays 40% (10% directly plus 30% through bank loan)."
  - q: "Who is eligible?"
    a: "Component A: farmers with barren land. Component B: farmers without grid connection. Component C: farmers with grid-connected agricultural pumps."
  - q: "What pump capacities are funded?"
    a: "0.5 HP to 7.5 HP for standalone pumps. Higher capacity through Component C feeder solarisation."
  - q: "How does Component A work?"
    a: "Farmer leases barren land to developer. Developer installs solar plant. Plant sells power to DISCOM at FIT. Farmer gets land lease."
  - q: "How does Component B work?"
    a: "Standalone solar pump replaces diesel pump. Farmer pays 10% capex. Government covers 30%. Bank loan 30%. State subsidy 30%."
  - q: "How does Component C work?"
    a: "Existing grid-connected agricultural pumps solarised through feeder-level or individual installations. DISCOM purchases surplus."
  - q: "Are PM-KUSUM components ALMM-required?"
    a: "Yes. ALMM-listed equipment is required for subsidy disbursement."
  - q: "Is net metering applicable?"
    a: "For Component C with grid-connected pumps, yes. Component B standalone does not require it."
  - q: "What is the typical farmer payback?"
    a: "3 to 5 years for standalone pumps, due to fuel cost replacement and subsidy."
  - q: "Where does Component A solar power go?"
    a: "Sold to local DISCOM at a state-determined feed-in tariff, typically ₹3.10 to ₹4.50/kWh."
author: "Nirav Dhanani"
---

## What is PM-KUSUM

PM-KUSUM, Pradhan Mantri Kisan Urja Suraksha evam Utthaan Mahabhiyan, is the Government of India's flagship scheme for agricultural solar, launched in March 2019. It targets 35 GW of solar capacity in agriculture by 2026 through three components: A, B and C. For a full walkthrough of subsidy amounts and state-wise application steps, see the [KUSUM Yojana Agricultural Solar Subsidy Complete Guide 2026](/blog/kusum-yojana-agricultural-solar-subsidy/).

The scheme addresses three distinct agricultural energy challenges: barren farm land that can host solar plants (Component A), farmers without grid access who rely on diesel pumps (Component B), and grid-connected pumps that subsidise DISCOMs (Component C). Each component has its own subsidy structure, eligible technology and implementation pathway.

Implementation runs through state agencies (state nodal agencies for renewable energy) with central financial support from [MNRE](/glossary/mnre/). Banks provide concessional finance for the farmer share of capex. [ALMM-listed](/glossary/almm-list/) equipment is required for subsidy disbursement.

> **TL;DR**
> - PM-KUSUM targets 35 GW of agricultural solar by 2026 across three components: A (decentralised plants), B (standalone pumps) and C (grid-connected pump solarisation).
> - Component A targets 10 GW on barren farm land, Component B targets 14 lakh standalone pumps, and Component C targets 35 lakh grid-connected pump solarisations.
> - Subsidy covers up to 60 percent of capex, split 30 percent central and 30 percent state, with the farmer paying 10 percent plus a 30 percent bank loan.
> - Launched in March 2019, the scheme requires ALMM-listed modules and inverters for subsidy disbursement.
> - Component B leads adoption at roughly 24 percent of target, while Components A and C lag due to land aggregation and DISCOM coordination delays.
> - Component A power is sold to the local DISCOM at a state-set [feed-in tariff](/glossary/feed-in-tariff/), typically ₹3.10 to ₹4.50 per kWh.

## PM-KUSUM components

| Component | Description | Target |
|---|---|---|
| Component A | Decentralised solar plants on farmer land | 10 GW |
| Component B | Standalone solar pumps | 14 lakh pumps |
| Component C | Grid-connected pump solarisation | 35 lakh pumps |

## Why PM-KUSUM matters

Diesel replacement. 35 lakh diesel pumps consume billions of litres of fuel annually. Solar pumps eliminate this.

Farmer income. Solar offers free electricity plus surplus revenue.

DISCOM relief. Agricultural subsidy is one of the largest [DISCOM](/glossary/discom/) losses. Solarisation reduces it.

Carbon abatement. Estimated 30 to 50 million tonnes CO2 saved annually at target capacity.

Rural electrification. Component B brings electricity where the grid does not reach, often paired with an [off grid inverter](/glossary/off-grid-inverter/) for standalone pump loads. Sizing the pump controller correctly matters as much as the subsidy; see [Solar Inverter for Water Pump, KUSUM-Compatible Picks](/blog/solar-inverter-for-water-pump/) for HP-to-kW conversion and inrush current guidance.

Land utilisation. Component A monetises barren land.

## Technical deep dive

### Component A details

| Aspect | Detail |
|---|---|
| Eligible land | Barren or fallow farm land |
| Capacity | 500 kW to 2 MW per plant |
| Tariff | State FIT, typically ₹3.10 to ₹4.50/kWh |
| Land lease | Farmer receives ₹0.30 to ₹1/kWh equivalent |
| Developer | Selected through state tender |
| Subsidy | None (commercial PPA model) |

### Component B details

| Aspect | Detail |
|---|---|
| Pump capacity | 0.5 to 7.5 HP |
| Capex | ₹1.5 to ₹4 lakh per pump |
| Central subsidy | 30 percent |
| State subsidy | 30 percent |
| Farmer contribution | 10 percent (with 30 percent bank loan) |
| Equipment | DC submersible or surface pump with VFD |

### Component C details

| Aspect | Detail |
|---|---|
| Eligible pumps | Existing grid-connected agricultural |
| Subsidy structure | Similar to Component B |
| Surplus power | Sold to DISCOM at FIT |
| [Net metering](/glossary/net-metering/) | Applicable for individual pump solarisation |
| Feeder solarisation | Aggregate solar at substation level |

### Equipment requirements

- ALMM-listed modules and inverters
- BIS-certified balance of system
- Pump motor compliant with MNRE specs
- VFD-based variable frequency for direct solar coupling
- Anti-theft features

## Real-world applications

Replacing diesel pumps in rural Maharashtra, Rajasthan, Gujarat, MP and Karnataka.

Decentralised 1 MW solar plants on farm fallows.

Feeder-level solarisation of agricultural connections.

Tribal and remote village solar pumps.

Drip and micro-irrigation solar pumps.

## Implementation status (2026 approximation)

| Component | Installed (approximate) | Target | % achieved |
|---|---|---|---|
| Component A | 0.7 GW | 10 GW | 7% |
| Component B | 3.4 lakh pumps | 14 lakh | 24% |
| Component C | 1.5 lakh pumps | 35 lakh | 4% |

Component B shows fastest adoption; A and C have struggled with land aggregation and DISCOM coordination.

## Advantages

Significant capex subsidy.

Diesel cost elimination.

Surplus power monetisation.

Land monetisation for Component A.

DISCOM AT&C improvement.

## Limitations

Component A land aggregation challenges.

DISCOM payment delays affecting FIT economics.

Service network gaps in remote areas.

ALMM and supply chain bottlenecks.

State-level implementation variation.

## Common problems

Bank loan disbursement delays.

ALMM-listed equipment availability in remote districts.

Pump theft from unattended fields.

Soiling reducing yield in dusty regions.

DISCOM net metering delays for Component C.

## Best practices

Verify state agency empanelment.

Choose ALMM-listed equipment.

Plan anti-theft measures for remote pumps.

Schedule annual maintenance and cleaning.

Maintain pump performance logs.

## Indian market context

State nodal agencies for renewable energy implement PM-KUSUM at the state level.

Major participating states: Maharashtra, Rajasthan, Gujarat, Madhya Pradesh, Karnataka, Andhra Pradesh, Punjab, Haryana.

MNRE tracks scheme progress monthly through state-wise dashboards.

Banks under PM-KUSUM include NABARD, SBI, BoB and rural cooperative banks.

PLI Phase II includes pump-specific manufacturing incentives.

## Standards and certifications

| Standard | Scope |
|---|---|
| MNRE PM-KUSUM scheme guidelines | Implementation framework |
| MNRE technical specifications for solar pumps | Equipment requirements |
| ALMM | Module and inverter listing |
| BIS certification | Quality compliance |
| IS 7396 / 9283 | Submersible pump standards |

## Common mistakes

Underestimating pump load profile.

Choosing pumps not in MNRE specs.

Skipping anti-theft hardware.

Inadequate maintenance schedule.

Not signing net metering for Component C.

## Key takeaways

PM-KUSUM is India's largest agricultural solar scheme with 35 GW target by 2026. Three components address decentralised solar plants on farm land, standalone solar pumps and grid-connected pump solarisation. Subsidies cover 60 percent of capex through central and state contributions. Component B leads adoption with over 3 lakh pumps installed. Component A and C face execution challenges around land aggregation and DISCOM coordination.

## Need PM-KUSUM solar pump installation?

QBits Energy installs ALMM-compliant solar pumps and decentralised solar plants under PM-KUSUM across participating Indian states.

## Sources

- PM-KUSUM Scheme Guidelines, MNRE.
- MNRE Technical Specifications for Solar Pumps.
- State Nodal Agency implementation documents.
- ALMM listing for solar pumps and inverters.
- MNRE Annual Reports.
- IS 7396, IS 9283, submersible pump standards.
- NABARD financing guidelines for solar pumps.
- IEA Renewables 2024 on agricultural solar.
- IRENA Renewable Energy in Agriculture.
