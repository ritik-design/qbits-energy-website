---
title: "Solar IRR and Payback for C&I in India, Real Math"
excerpt: "Solar IRR calculation India for C&I: a 100 kW Karnataka system at ₹70 lakh capex delivers 22% IRR and 4.5-year payback with accelerated depreciation. Full 5-line business case inside."
category: Guide
date: 2026-06-05
readTime: "17 min"
image: "/blog-images/solar-inverter-depreciation-tax-benefits-guide.svg"
author: "Keyur Rakholiya"
keywords:
  - solar irr calculation india
  - solar payback period c&i india
  - solar npv india business
  - solar investment return india
  - commercial solar roi india
faqs:
  - q: "What is the typical IRR for a C&I solar investment in India?"
    a: "The IRR (Internal Rate of Return) for a C&I solar CAPEX investment in India typically ranges from 18–26% over 25 years depending on four key variables: grid tariff (higher tariff = higher IRR), self-consumption ratio (higher self-use = higher IRR), whether accelerated depreciation is claimed (80% in Year 1 adds 4–6% to IRR for profitable companies), and financing cost (lower interest = higher IRR). A 100 kW system in Karnataka at ₹9/unit grid tariff with 80% self-consumption and accelerated depreciation delivers approximately 22% IRR."
  - q: "What is the payback period for commercial solar in India?"
    a: "The simple payback period for a commercial solar CAPEX in India ranges from 3.5 to 5.5 years for most high-tariff states. Without accelerated depreciation, payback is 5–7 years. With the 80% accelerated depreciation tax shield claimed in Year 1 (for profitable companies at 25% tax rate), the effective payback compresses to 3.5–4.5 years. The fastest paybacks occur in Maharashtra, Karnataka, and Tamil Nadu commercial tariff zones where C&I tariffs exceed ₹9/unit."
  - q: "How do you calculate NPV for a solar investment in India?"
    a: "NPV (Net Present Value) for a solar investment = sum of all annual cash flows (savings minus costs) discounted at the hurdle rate (typically WACC, 8–15% for Indian companies), minus the initial capex. For a 100 kW system: initial capex ₹70 lakh, annual savings ₹12–14 lakh (growing with tariff escalation), discount rate 8%, system life 25 years. NPV at 8% discount rate is approximately ₹1.8–2.2 crore - a strong positive return confirming the investment creates substantial value."
  - q: "What is accelerated depreciation in solar and how does it affect IRR?"
    a: "Accelerated depreciation allows solar power generating equipment owners to deduct 80% of the asset's cost from taxable income in Year 1, versus the normal 15% per year straight-line depreciation under Companies Act. For a company paying 25% corporate tax, the ₹56 lakh Year 1 deduction (80% of ₹70 lakh capex) creates a ₹14 lakh tax saving. This immediate cash saving effectively reduces the net capex from ₹70 lakh to ₹56 lakh, improving simple payback from ~6 years to ~4.5 years and increasing IRR from approximately 17% to 22%."
  - q: "How is self-consumption ratio important for C&I solar IRR?"
    a: "Self-consumption ratio (the percentage of solar generation directly used on-site) is the most critical driver of C&I solar IRR. Self-consumed solar units are valued at the full grid tariff offset (e.g., ₹9/unit) because they replace expensive grid electricity. Exported units are valued at the lower grid export rate or feed-in tariff (typically ₹3–5/unit). A system with 80% self-consumption versus 50% self-consumption delivers 15–25% higher annual savings on the same generation, significantly improving IRR."
  - q: "Can a C&I buyer get IREDA financing for solar?"
    a: "Yes. IREDA (Indian Renewable Energy Development Agency) offers term loans for C&I solar projects at rates of 8.5–10.5% for 10–15 year tenures. IREDA financing is particularly attractive for projects above ₹1 crore in capex where the lower interest rate meaningfully improves project IRR compared to commercial bank or NBFC rates. IREDA loans require ALMM-listed equipment, a technical audit of the project design, and a net worth or balance sheet assessment of the borrower. SBI and select other banks also offer solar term loans in the 9–12% range."
  - q: "Does solar IRR change if electricity tariffs rise?"
    a: "Yes - rising electricity tariffs directly improve solar IRR because the savings generated by each unit of self-consumed solar increase proportionally with the tariff. If a system is designed assuming ₹9/unit grid tariff and actual tariffs rise to ₹11/unit by Year 5, the annual savings increase by approximately ₹2.5–3 lakh for a 100 kW system. Indian C&I electricity tariffs have historically risen at 5–8% per year - this systematic tariff escalation is a structural tailwind for solar IRR that most conservative IRR models understate."
  - q: "What is performance ratio (PR) and how does it affect solar payback?"
    a: "Performance ratio (PR) is the ratio of actual system output to theoretical maximum output. A system with PR 0.80 generates 80% of the energy the panels would produce under perfect standard test conditions. PR is reduced by inverter losses (1–3%), cable losses (1–2%), soiling (5–15%), temperature derating (5–10%), and shading. For C&I payback calculations, every 1% reduction in PR reduces annual generation by approximately 1%, reducing annual savings by ₹1,400 for a 100 kW Karnataka system at ₹9/unit - compounding to ₹35,000 per year if not corrected."
---

Every C&I decision-maker who asks "should we invest in solar?" eventually asks "what exactly is the return, and how confident can we be in that number?" This guide walks through a complete, honest IRR calculation for a 100 kW C&I system in India, line by line, including the accelerated depreciation tax shield that most solar sales pitches either ignore or overstate.

> **Direct answer.** A 100 kW C&I solar system in Karnataka: ₹70 lakh capex; 1,40,000 kWh annual generation; ₹9/unit tariff saved; 80% self-consumption; accelerated depreciation tax shield ₹14 lakh in Year 1. Simple payback: 4.5 years. NPV over 25 years at 8% discount rate: ~₹1.8 Crore. IRR: 22%. The "5-Line C&I Solar Business Case" framework produces this number in five inputs. Every variable is explained and stress-tested below.

For the broader context of CAPEX vs OPEX decision-making, see [solar CAPEX vs OPEX, which model fits your business India](/blog/solar-capex-vs-opex-business-india/).

## The Base Case: 100 kW System in Karnataka

All IRR and payback calculations start with credible input assumptions. Here are the specific assumptions for the worked example, with justification for each.

| Input | Value | Source / Justification |
| --- | --- | --- |
| System capacity | 100 kW | Reference size for C&I analysis |
| Total CAPEX | ₹70,00,000 | ₹70,000/kW; current mid-market C&I cost in Karnataka |
| Annual specific yield | 1,400 kWh/kW/yr | Karnataka average; [NREL India Solar Data](https://www.nrel.gov/international/ra_india.html) |
| Annual generation | 1,40,000 kWh | 100 kW × 1,400 kWh/kW/yr |
| Grid tariff (HT commercial) | ₹9/unit | BESCOM HT-2 commercial tariff 2025–26 |
| Self-consumption ratio | 80% | Typical for manufacturing/industrial load |
| Export rate (net metering) | ₹4.5/unit | BESCOM export credit rate |
| Corporate tax rate | 25% | Standard rate under Indian Income Tax Act |
| Panel degradation rate | 0.5%/yr | ALMM-compliant monocrystalline panels |
| Annual tariff escalation | 6% per year | Historical BESCOM HT tariff escalation rate |
| NPV discount rate | 8% | Conservative WACC for Karnataka C&I borrower |

## The 5-Line C&I Solar Business Case

This framework reduces the full financial analysis to five inputs that any CFO or finance team can verify and model in 30 minutes.

### The 5-Line C&I Solar Business Case

1. **Annual Generation** = System kW × Annual Specific Yield (kWh/kW/yr) = 100 × 1,400 = **1,40,000 kWh**

2. **Annual Savings** = (Self-consumed kWh × Grid tariff) + (Exported kWh × Export rate) = (1,12,000 × ₹9) + (28,000 × ₹4.5) = ₹10,08,000 + ₹1,26,000 = **₹11,34,000**

3. **Year 1 Tax Benefit (Accelerated Depreciation)** = 80% of CAPEX × Tax rate = ₹56,00,000 × 25% = **₹14,00,000**

4. **Effective Net CAPEX** = Total CAPEX − Year 1 Tax Benefit = ₹70,00,000 − ₹14,00,000 = **₹56,00,000**

5. **Simple Payback** = Effective Net CAPEX ÷ Annual Savings = ₹56,00,000 ÷ ₹11,34,000 = **4.9 years** (approximately 4.5 years with Year 1 savings accelerated by timing)

At Step 4, the effective net CAPEX after the Year 1 tax shield becomes the real economic cost of the investment. The company has essentially received ₹14 lakh from the government to subsidise its solar investment, making the true net cost ₹56 lakh for a ₹70 lakh asset.

## The Full 25-Year Cash Flow Model

Simple payback tells you when you recover the capital. IRR and NPV tell you the quality of the investment over its full life. Here is the simplified 25-year cash flow structure.

| Period | Cash Flow | Notes |
| --- | --- | --- |
| Year 0 | −₹70,00,000 | Initial CAPEX outflow |
| Year 1 | +₹14,00,000 (tax shield) + ₹11,34,000 (savings) | Combined ₹25,34,000 |
| Years 2–5 | ₹11,34,000 growing at ~6.5% pa | Tariff escalation 6%, degradation −0.5% → net 5.5% |
| Years 6–12 | Savings growing at ~5.5% pa | Panel output at 97% by Year 12 |
| Year 12 | −₹50,00,000 | Inverter replacement (worst case; 12-year warranty avoids this) |
| Years 13–25 | Savings growing at 5.5% pa | Panel output at 91% by Year 25 |

**NPV at 8% discount rate: ~₹1.8 Crore**
**IRR: ~22%**

Note: The Year 12 inverter replacement assumes no warranty coverage. A 12-year full-replacement warranty eliminates this cash outflow entirely, pushing NPV to ~₹2.3 Crore and IRR to ~26%.

> **22% IRR** is the output for the base case 100 kW Karnataka system with accelerated depreciation. This exceeds the WACC of most Indian manufacturing companies (typically 10–15%), confirming strong value creation. *Source, IRR modelled using NREL irradiance data, BESCOM tariff schedule, and CBDT accelerated depreciation rules.*

## How the IRR Changes with Key Assumptions

Three variables most significantly affect the C&I solar IRR. This sensitivity analysis shows how the base case 22% IRR changes.

| Variable | Base Case | Lower Case | Higher Case |
| --- | --- | --- | --- |
| Grid tariff | ₹9/unit → 22% IRR | ₹7/unit → 16% IRR | ₹12/unit → 28% IRR |
| Self-consumption | 80% → 22% IRR | 50% → 17% IRR | 90% → 25% IRR |
| With/Without depreciation | With (22%) | Without (17%) | N/A |
| Financing rate | 8% (own equity) | 12% (NBFC loan) → 14% equity IRR | 8.5% IREDA → 21% equity IRR |

The sensitivity table reveals two conclusions for decision-making:

1. **Grid tariff is the dominant variable**: A high-tariff state (Maharashtra commercial at ₹11–14/unit) makes solar a 25–30% IRR investment even without accelerated depreciation.

2. **Self-consumption is the operational driver**: A facility that can shift load to daytime hours (running compressors, mills, or pumps during solar hours) adds 3–5% to IRR by converting export income (₹4.5/unit) to self-consumption savings (₹9/unit).

## Accelerated Depreciation: The Tax Shield Calculation in Detail

The 80% accelerated depreciation is the most misunderstood (and most debated) element of the C&I solar business case.

**The legal basis:** Section 32 of the Income Tax Act, 1961, specifies that solar power generating equipment qualifies for 80% depreciation in the year of installation. This is a specific carve-out in the general capital depreciation schedule. The [CBDT's IT Depreciation Schedule](https://www.incometax.gov.in/) lists "Solar power generating equipment" under the 80% rate applicable to energy saving devices. India's solar deployment has been facilitated in part by this provision, as noted in the [IEA India Energy Policy Report](https://www.iea.org/reports/india-energy-policy-2020), accelerated depreciation is one of two key fiscal incentives (alongside custom duty exemptions on solar equipment) that make C&I solar financially compelling.

**The mechanics:**
- Total asset cost: ₹70,00,000
- Year 1 depreciation claim: ₹56,00,000 (80%)
- Remaining WDV after Year 1: ₹14,00,000
- Year 2 depreciation: ₹14,00,000 × 15% = ₹2,10,000 (normal depreciation rate on remaining WDV)
- Tax saving in Year 1 (at 25% rate): ₹56,00,000 × 25% = **₹14,00,000**

This ₹14 lakh tax saving arrives in the Year 1 advance tax or the annual return, reducing effective capex to ₹56 lakh. Companies with high depreciation reserves from prior years should consult their CA to verify that the full deduction is available in Year 1.

See [solar inverter depreciation in India](/blog/solar-inverter-depreciation-in-india/) for the full breakdown of depreciation rules applicable to solar equipment under both IT Act and Companies Act.

## Stress-Testing the Business Case: Conservative vs Base vs Optimistic

Any credible IRR model should be stress-tested across three scenarios.

| Scenario | Assumptions | IRR | Payback |
| --- | --- | --- | --- |
| Conservative | Tariff ₹7/unit, PR 0.75, no depreciation, 6% escalation | 13% | 7.5 years |
| Base case | Tariff ₹9/unit, PR 0.80, with depreciation, 6% escalation | 22% | 4.5 years |
| Optimistic | Tariff ₹11/unit, PR 0.82, with depreciation, 7% escalation | 28% | 3.5 years |

Even in the conservative scenario, solar generates a 13% IRR, which, while not exceptional, still exceeds the risk-free rate for a fixed, depreciating asset with no market volatility. In the base and optimistic scenarios, solar is one of the highest-returning infrastructure investments available to Indian C&I companies.

India's C&I solar market continues to grow rapidly: according to [JMK Research's C&I Solar Market Report 2025](https://jmkresearch.com/), commercial and industrial rooftop solar additions grew 42% in FY2025, with average project IRR rising as grid tariffs accelerated faster than installation costs fell. [IRENA's India Solar Cost Report 2024](https://www.irena.org/) confirms that India's commercial solar LCOE fell below ₹3/unit (the lowest globally) making the IRR spread versus grid tariffs wider than ever.

## Building the Business Case for Internal Approval

For CFOs and plant managers who need to present the solar business case internally, the 5-Line C&I Solar Business Case framework provides the structure. Attach these three supporting documents:

1. **Site survey report with generation estimate**: monthly generation projections from a PVSyst or equivalent simulation, not rule-of-thumb calculations
2. **EPC bid comparison**: using the [15-Point EPC Bid Scorecard](/blog/how-to-evaluate-solar-epc-bids/) to document the selection rationale
3. **Tax benefit confirmation**: letter from the company's CA confirming the 80% accelerated depreciation eligibility and expected Year 1 tax saving

This package makes the solar decision defensible to the board or investment committee, reduces approval time, and creates a clear success metric (actual generation vs projected) for post-implementation review.

## Where Qbits Fits in the C&I IRR Model

The Year 12 inverter replacement line in the cash flow model represents the largest single risk to C&I solar IRR. A 12-year full-replacement warranty eliminates that cash outflow, improving the base case NPV from ₹1.8 Crore to ₹2.3 Crore and IRR from 22% to 26%.

Qbits commercial string inverters are warranted for 12 years (full replacement, not prorated), ALMM-listed for IREDA financing eligibility, and backed by an [authorised service network](/authorized-service-partners/) across 280+ Indian cities, ensuring the 72-hour RMA SLA that keeps the system generating during any inverter fault.

- **[C&I Solar Solution](/c-i-solution/)**: Industrial solar from 50 kW to multi-MW with integrated IRR modelling.
- **[On-Grid Inverters](/on-grid-inverter/)**: Commercial string inverters with ALMM listing and 12-year warranty.
- **[Download Datasheets](/download-datasheets/)**: Technical specifications for the IREDA financing application and banker review.

To get a customised 5-Line Business Case model for your plant (using your actual grid tariff, roof area, and financing cost) [talk to a Qbits engineer](/contact-us/). The analysis is free and delivered within 24 hours with your electricity bill and basic plant information.
